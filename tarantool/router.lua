...
local function first_crud_object(result)
    ...
    if row == nil then return box.NULL end
    local object = {}
...
function api.user_update(id, changes)
    ...
    if #operations == 0 then return api.user_get(id) end
...
local function replicasets()
    ...
    if routes == nil then error(tostring(err)) end
    for _, replicaset in pairs(routes) do
        table.insert(result, replicaset)
    end
    if #result == 0 then error('no replica sets are configured') end
    return result
end

local function decode_cursor(cursor)
    if cursor == nil or cursor == box.NULL or cursor == '' then return 0, 1 end
    if type(cursor) ~= 'string' then error('invalid cursor') end
    local page_number, last_id = string.match(cursor, '^v1:(%d+):(%d+)$')
    if page_number == nil then error('invalid cursor') end
...
local function scatter_call(mode, function_name, arguments)
    ...
    for _, target in ipairs(targets) do
        ...
        fiber.create(function()
            ...
            local ok, result, err = pcall(call, replicaset, function_name,
                arguments, {timeout = common.read_timeout})
    ...
    for _ = 1, #targets do
        ...
        if response == nil then error('scatter-gather read timed out') end
        if not response.success then error(tostring(response.err)) end
...
function api.users_page(cursor, requested_limit)
    ...
    if select_err ~= nil then error(tostring(select_err)) end
    ...
    while #merged > limit do table.remove(merged) end
    local next_cursor = box.NULL
    if has_more then next_cursor = 'v1:' .. (current_page + 1) .. ':' .. merged[#merged].id end
    ...
    return {
        ...
        -- An exact final-page cursor requires an order-statistics index.
        -- Returning NULL keeps this endpoint O(shards * page_size).
        lastCursor = box.NULL,
    }
end
...
local function auth_id(where)
    for _, condition in ipairs(where or {}) do
        if condition.field == 'id' and
            (condition.operator == nil or condition.operator == 'eq') then
...
function api.auth_find_many(model, where, limit, offset, sort_by)
    if sort_by == box.NULL then sort_by = nil end
    ...
    if id ~= nil then
        local shard_rows, err = vshard.router.callro(auth_bucket(model, id),
            'storage_api.auth_find',
            {model, where or {}, requested + skipped, sort_by or box.NULL},
            {timeout = common.read_timeout})
        if shard_rows == nil then error(tostring(err)) end
        rows = shard_rows
    else
        for _, shard_rows in ipairs(scatter_callro('storage_api.auth_find',
            {model, where or {}, requested + skipped, sort_by or box.NULL})) do
            for _, row in ipairs(shard_rows) do table.insert(rows, row) end
        end
    end
    if sort_by ~= nil then
        table.sort(rows, function(left, right)
            if left[sort_by.field] == right[sort_by.field] then return left.id < right.id end
            if left[sort_by.field] == nil or left[sort_by.field] == box.NULL then return false end
            if right[sort_by.field] == nil or right[sort_by.field] == box.NULL then return true end
...
function api.auth_count(model, where)
    local id = auth_id(where)
    if id ~= nil then
        local count, err = vshard.router.callro(auth_bucket(model, id),
            'storage_api.auth_count', {model, where or {}},
            {timeout = common.read_timeout})
        if count == nil then error(tostring(err)) end
        return count
    end
    local total = 0
    for _, count in ipairs(scatter_callro('storage_api.auth_count',
        {model, where or {}})) do total = total + count end
    return total
end
...
local function auth_mutate_one(model, where, changes, consume, increments)
    local id = auth_id(where)
    if id ~= nil then
        local row, err = vshard.router.callrw(auth_bucket(model, id),
    ...
    local found = nil
    for _, row in ipairs(scatter_callrw('storage_api.auth_update_one',
        {model, where, changes or {}, consume or false, increments or {}})) do
        if row ~= nil and row ~= box.NULL then
            if found ~= nil then error('auth uniqueness violation across shards') end
            found = row
        end
    end
    return found
...
function api.auth_update_many(model, where, changes, remove)
    local count = 0
    for _, changed in ipairs(scatter_callrw('storage_api.auth_update_many',
        {model, where, changes or {}, remove or false})) do count = count + changed end
    return count
end

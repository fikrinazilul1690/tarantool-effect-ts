...
export const RequestSchemaErrorLive = HttpApiMiddleware.layerSchemaErrorTransform(
  RequestSchemaError,
  (schemaError) => {
    if (isRequestError(schemaError.kind)) {
      return Effect.fail(validationErrorResponse(
...
function isRequestError(
  ...
  return kind === 'Params' || kind === 'Headers' || kind === 'Query' || kind === 'Payload';
}
...
function groupIssuesByField(
  ...
  for (const formatted of formatIssues(issue).issues) {
    const field = formatted.path?.map((segment) =>
      String(typeof segment === 'object' ? segment.key : segment)).join('.') || '_root';
